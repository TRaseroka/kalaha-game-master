...
while (!isGameOver()) {
    ...
    private void captureIfLastPitIsOwnEmptyPit(final int index) {
        if (pit_contains_one_stone(index) && is_a_regular_pit(index) && lands_in_players_own_pit(index)) {
    ...
    private boolean pit_contains_one_stone(final int index) {
        return getStonesInPit(index) == 1;
    }

    private boolean is_a_regular_pit(final int index) {
        return index != getIndexKalahaSouth() && index != getIndexKalahaNorth();
    }

    private boolean lands_in_players_own_pit(final int index) {
